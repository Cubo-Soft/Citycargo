...
$(document).ready(function () {
    ...
    $("#modificar2").on("click", function () {
        ...
        datos.id_es_entidades_seguridad_social = $("#es_entidades_seguridad_social1").val();
        datos.id_es_nombres_estados = $("#es_nombres_estados1").val();
        datos.id = $("#id_es_nombres_entidades_seguridad_social").val();
        modificarESNombresEntidadesSeguridadSocial(datos, 1);
    });

    $("#crear2").on("click", function () {

        if ($("#nombre2").val().length === 0) {
            
            $("#nombre2").focus();
            $("#divMensajes").html('<div class="alert alert-danger">Por favor ingrese un nombre válido</div>');

        } else if ($("#es_entidades_seguridad_social1").val() === '-1') {

            $("#es_entidades_seguridad_social1").focus();
            $("#divMensajes").html('<div class="alert alert-danger">Por favor seleccione el tipo de entidad</div>');
            ...
            datos.id_es_entidades_seguridad_social = $("#es_entidades_seguridad_social1").val();
            datos.id_es_nombres_estados = $("#es_nombres_estados1").val();
            crearESNombresEntidadesSeguridadSocial(datos, 1);
        }
    });

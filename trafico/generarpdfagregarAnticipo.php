...
if ($cantidadVeces <= 0) {
    ...
    $consulta = "select guias from posiblesanticipos where idservicio=".$_POST["idservicio"].";";    
    $prepare = $conexion->prepare($consulta);
    $prepare->execute();    
    $arreglo = $prepare->fetchAll();
    ...
    if ($_POST["varEmp"] === '0') {
        $varEmp = '0';
        $guia = $arreglo[0]["guias"];
    } else if ($_POST["varEmp"] === '1') {
        $varEmp = '1';
        for ($index2 = 0; $index2 < count($arreglo); $index2++) {
            $guia = $guia . '-' . $arreglo[$index2]["guias"];
        }
        $guia = substr($guia, 1);
    } else {
...
//si cuando venga en post viene un val_ant_id# quiere decir que a ese conductor
//se le hicieron adelantos, por lo tanto no se debe tener en cuenta dichos 
//adelantos para guardar en la base.
//este ciclo indica donde inicia la busqueda del otro adelanto
    for ($i = 0; $i < 40; $i++) {
        $val_ant_id .= $i;

        if (isset($_POST[$val_ant_id])) {
            $vlrIni = $i + 1;
        }
        $val_ant_id = "val_ant_id";
    }
    ...
    $valorTotalServicio .= $vlrIni;
//creo el nuevo anticipo en la tabla totalesanticipos
    $consulta = "insert into totalesanticipos values (null," . $_POST[$valorTotalServicio] . ",'A'," . $_POST['numeroAnticipo'] . "," . $_POST['fechaApertura'] . ",0);";
    $prepare = $conexion->prepare($consulta);
    $prepare->execute();
    $consulta = "select val_id from totalesanticipos where val_numeroAnticipo=" . $_POST['numeroAnticipo'] . ";";
    $prepare = $conexion->prepare($consulta);
    $prepare->execute();
    $arreglo = $prepare->fetchAll();
...
//inserto en la tabla valoresAnticipos cada uno de los anticipos que vienen aqui
    ...
    for ($index = $vlrIni; $index < 40; $index++) {
        ...
        if (intval(@$_POST[$nitEmp]) > 0 && $index === $vlrIni) {
            $consulta = "select m.mun_nombre "
                    . "from servicios as s, municipios as m "
                    . "where s.idciudadorigen=m.mun_id "
                    . "and s.idservicio=" . $_POST[$idservicio] . ";";
        }

        if (intval(@$_POST[$nitEmp]) > 0) {
...
//en caso de que los valores vengan vacios se redirige a cotizacion.php con el id del conductor bien
//sea creado o el que ya existia
